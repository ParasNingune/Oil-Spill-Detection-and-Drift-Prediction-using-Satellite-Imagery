...
# Drift simulation imports (optional - will warn if not available)
try:
    import copernicusmarine as cm
    COPERNICUSMARINE_AVAILABLE = True
except ImportError:
    COPERNICUSMARINE_AVAILABLE = False
    print("Warning: copernicusmarine not installed. Drift simulation will not work.")
...
# ============================================================================
ENABLE_DRIFT_SIMULATION = COPERNICUSMARINE_AVAILABLE and OPENDRIFT_AVAILABLE
COPERNICUS_USERNAME = os.environ.get('COPERNICUS_USERNAME', '')  # Set via environment variable
COPERNICUS_PASSWORD = os.environ.get('COPERNICUS_PASSWORD', '')  # Set via environment variable
    ...
    try:
        ...
        if has_oil:
            ...
            if ENABLE_DRIFT_SIMULATION:
                ...
                response['drift_animation_url'] = f'/api/animations/drift_animation_{timestamp}.mp4'
        ...
        if has_oil and ENABLE_DRIFT_SIMULATION:
            print(f"✓ Starting background drift simulation thread...")
...
def health_check():
    ...
    return jsonify({
        ...
        'copernicusmarine_available': COPERNICUSMARINE_AVAILABLE,
        'opendrift_available': OPENDRIFT_AVAILABLE,
        'folium_available': FOLIUM_AVAILABLE,
        'copernicus_credentials_set': bool(COPERNICUS_USERNAME and COPERNICUS_PASSWORD)
    }), 200
...
def check_animation_status(filename):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            print(f"[⏱️  Animation Status] {status_icon} File found: {filename} ({file_size} bytes) - Ready: {is_ready}")
            ...
            print(f"[⏱️  Animation Status] ⏳ Still generating: {filename}")
            return jsonify({
                'ready': False,
                'filename': filename,
                'status': 'generating'
        ...
        print(f"[⏱️  Animation Status] ✗ Error: {str(e)}")
        import traceback
        traceback.print_exc()
...
if __name__ == '__main__':
    ...
    print(f"U-Net Model Status: {'✓ Loaded' if unet_model else '✗ Not Loaded'}")
    print(f"Model Path: {MODEL_PATH}")
    print(f"\nDrift Simulation Status: {'✓ Enabled' if ENABLE_DRIFT_SIMULATION else '✗ Disabled'}")
    if ENABLE_DRIFT_SIMULATION:
        print(f"  - Copernicusmarine: {'✓ Available' if COPERNICUSMARINE_AVAILABLE else '✗ Not available'}")
        print(f"  - OpenDrift: {'✓ Available' if OPENDRIFT_AVAILABLE else '✗ Not available'}")
        print(f"  - Folium: {'✓ Available' if FOLIUM_AVAILABLE else '✗ Not available'}")
        print(f"  - Copernicus Credentials: {'✓ Set' if (COPERNICUS_USERNAME and COPERNICUS_PASSWORD) else '✗ Not set (use env vars)'}")
        print(f"  - Simulation Duration: {DRIFT_SIMULATION_DURATION_HOURS} hours")
        ...
        if not OPENDRIFT_AVAILABLE:
            print("    - pip install opendrift")
        if not FOLIUM_AVAILABLE:
            print("    - pip install folium")
    print("="*70)
    print("\nEndpoints:")
    print("  • POST /api/predict - Detect oil and run drift simulation")
